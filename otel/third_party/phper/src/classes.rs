...
use crate::{
    ...
    functions::{Callable, Function, FunctionEntry, HandlerMap, Method, MethodEntity},
...
// Zend requires object-handler tables to live at least as long as every object
// that references them. Two immutable, process-lifetime tables cover the only
// class-dependent behavior here without allocating a table per PHP object.
static STATE_OBJECT_HANDLERS: OnceLock<usize> = OnceLock::new();
static CLONEABLE_STATE_OBJECT_HANDLERS: OnceLock<usize> = OnceLock::new();
...
impl<T: 'static> ClassEntity<T> {
    ...
    pub(crate) fn method_handlers(&self) -> impl Iterator<Item = (&CStr, Rc<dyn Callable>)> {
        self.method_entities.iter().filter_map(|method| {
            method
                .handler
                .as_ref()
                .map(|handler| (method.name.as_c_str(), handler.clone()))
        })
    }
...
pub(crate) unsafe extern "C" fn create_object(ce: *mut zend_class_entry) -> *mut zend_object {
    unsafe {
        ...
        // Set handlers. The table is shared by all objects with the same clone
        // behavior; allocating one here for every object leaks native memory
...
unsafe fn find_real_ce(mut ce: *mut zend_class_entry) -> Option<*mut zend_class_entry> {
    unsafe {
        // Internal classes registered by this module are their own real class
        // entry; only userland subclasses need to walk the parent chain.
        if (*ce).type_ as u32 == ZEND_INTERNAL_CLASS
            && (*ce).info.internal.module == crate::modules::global_module_entry() as *mut _
        {
            return Some(ce);
        }
        ...
        while !ce.is_null() {
            for entity in class_entities {
                if entity.bound_class.as_class_entry().as_ptr() == ce {
                    return Some(ce);
                }
            }
