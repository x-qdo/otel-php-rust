...
run_transport() {
    ...
    PHP_VERSION="${php_version}" docker compose --project-directory "${repo_root}" run --rm \
        ...
        -e 'OTEL_EXPORTER_OTLP_TIMEOUT=2000' \
        -e 'OTEL_BSP_MAX_QUEUE_SIZE=1' \
        -e 'OTEL_BSP_MAX_EXPORT_BATCH_SIZE=1' \
        -e 'OTEL_BSP_SCHEDULE_DELAY=60000' \
        -e 'OTEL_PHP_SHUTDOWN_TIMEOUT=100' \
        php php \
        -d extension=/usr/src/myapp/modules/otel.so \
        -d otel.cli.enabled=1 \
        tests/integration/batch_queue_full.php
}
