...
run_transport() {
    ...
    PHP_VERSION="${php_version}" docker compose --project-directory "${repo_root}" run --rm \
        ...
        -e 'OTEL_PHP_EXPORT_RETRY_MAX_ATTEMPTS=3' \
        -e 'OTEL_PHP_EXPORT_RETRY_INITIAL_BACKOFF=50' \
        -e 'OTEL_PHP_EXPORT_RETRY_MAX_ELAPSED=2000' \
        -e 'OTEL_LOGS_EXPORTER=none' \
        php php \
        -d extension=/usr/src/myapp/modules/otel.so \
        -d otel.cli.enabled=1 \
        tests/integration/batch_export_retry.php
}
